...
const MerchItem = () => {
  ...
  const chosenItem = merchItems && merchItems.filter((el) => el.id === id);
  useEffect(() => {
  ...
  const { getItemsMerch } = useContentful();
  useEffect(() => {
    getItemsMerch(setLoading).then((response) => {
      setMerchItems(response);
    });
  }, []);
      ...
      <HeaderLink>
        <LinkString>
          ...
          <Link to="/katalog-merch">
            <Main>Каталог мерча</Main>
          </Link>
          <Next src="/images/next_gray.svg" alt="asd" />{" "}
          <RedString>
            {chosenItem.length ? chosenItem[0].name : <></>}
          </RedString>
        </LinkString>
      ...
      {merchItems.length && (
        <RelinkingWrapper style={{ marginBottom: "200px" }}>
          ...
          <CardWrapper>
            <RelCardItem
              data={merchItems[0]}
              linkUrl={`katalog-merch/${merchItems[0].id}`}
            />
            {merchItems[1] ? (
              <RelCardItem
                data={merchItems[1]}
                linkUrl={`katalog-merch/${merchItems[1].id}`}
              />
            ) : (
              <RelCardItem
                data={merchItems[0]}
            ...
            {merchItems[2] ? (
              <RelCardItem
                data={merchItems[2]}
                linkUrl={`katalog-merch/${merchItems[2].id}`}
              />
            ) : (
              <RelCardItem
                data={merchItems[0]}
            ...
            <ResWrapperCard>
              {merchItems[3] ? (
                <RelCardItem
                  data={merchItems[3]}
                  linkUrl={`katalog-merch/${merchItems[3].id}`}
                />
              ) : (
                <RelCardItem
                  data={merchItems[0]}
