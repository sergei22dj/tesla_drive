...
const AccessoriesItem = ({ tags }) => {
  ...
  const chosenItem =
    accessoriesItems && accessoriesItems.filter((el) => el.id === id);
  useEffect(() => {
  ...
  const { getItemsAccessories } = useContentful();
  useEffect(() => {
    getItemsAccessories(setLoading).then((response) => {
      setAccessoriesItems(response);
    });
  }, []);

  // filter
  const applyFilters = () => {
    let updatedList = accessoriesItems;
  ...
  useEffect(() => {
    if (accessoriesItems.length) {
      applyFilters();
    }
  }, [accessoriesItems, selectedModel]);
          ...
          <Wrapper>
            <LWrapper>
              <ImgBar>
                ...
                />
                {chosenItem[0].image[1] && (
                  <ImgS
                    img={chosenItem[0].image[img2]}
                    onClick={() => [
                      setNumberImg(img2),
                      setNumberImgModal(img2),
                    ]}
                  />
                )}

                {chosenItem[0].image[2] && (
                  <ImgS
                    img={chosenItem[0].image[img3]}
                    onClick={() => [
                      setNumberImg(img3),
                      setNumberImgModal(img3),
                    ]}
                  />
                )}
              </ImgBar>
